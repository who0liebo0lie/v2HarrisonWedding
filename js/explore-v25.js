...
const categoryData={
  dining:{title:'What Are You Hungry For?',intro:'From quick poolside bites to a full dressed-up dinner, Harmony has something for every craving.',tabs:['All Dining','Included','Specialty','Quick Bites','Coffee & Tea','Late Nights'],items:[
    ['Windjammer Marketplace','Endless choices, fresh flavors, and ocean views.','assets/venues/windjammer.jpg','Included','Casual','Deck 16'],
    ['Main Dining Room','Multiple courses, polished service, and a perfect night together.','assets/venues/main-dining.jpg','Included','Dinner','Decks 3–5'],
    ['El Loco Fresh','Tacos, burritos, and pool-day fuel.','assets/venues/el-loco.jpg','Included','Quick Bites','Pool Deck'],
    ['Café Promenade','Coffee, pastries, sandwiches, and late-night snacks.','assets/venues/cafe-promenade.jpg','Included','Coffee & Tea','Royal Promenade'],
    ['Chops Grille','The celebratory steak dinner with premium cuts and classic sides.','assets/venues/chops.jpg','Specialty','Dinner','Central Park'],
    ["Jamie’s Italian",'Rustic Italian favorites designed for sharing.','assets/venues/jamies.jpg','Specialty','Dinner','Central Park'],
    ['Izumi Hibachi & Sushi','Sushi and hibachi tableside—dinner and a show.','assets/venues/izumi.jpg','Specialty','Dinner','Deck 4'],
    ['Sorrento’s','Pizza by the slice and a dependable late-night favorite.','assets/venues/sorrentos.jpg','Included','Late Nights','Royal Promenade']]},
...
function renderNeighborhood(key){
  ...
  title.textContent=n.title;description.textContent=n.description;image.src=n.image;image.alt=`${n.title} neighborhood`;venueHeading.textContent=`Venues in ${n.title}`; const selected=document.querySelector('#selected-neighborhood'); if(selected) selected.textContent=`${n.title} selected`;
  venueCards.innerHTML=n.venues.map(v=>`<article class="venue-card"><img src="${v.image}" alt="${v.name}"><div class="body"><span class="tag ${v.type==='Included'?'included':''}">${v.type}</span><h4>${v.name}</h4><p>${v.desc}</p><small>${v.location}${v.type==='Specialty'?' · Reservations may be recommended':''}</small></div></article>`).join('');
...
function renderDirectory(filter){
  ...
  grid.innerHTML=items.map(item=>`<article><img src="${item[2]}" alt="${item[0]}"><div><h3>${item[0]}</h3><p>${item[1]}</p><p class="directory-meta"><strong>${item[3]}</strong><span>• ${item[4]}</span><span>• ${item[5]}</span></p>${item[3]==='Specialty'?'<p class="directory-meta"><span>Reservations may be recommended</span></p>':''}</div></article>`).join('');
